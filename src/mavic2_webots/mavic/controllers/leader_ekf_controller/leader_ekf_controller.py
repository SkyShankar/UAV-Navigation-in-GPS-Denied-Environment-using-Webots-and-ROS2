...
class SensorPublisher(Node):
    def __init__(self, robot_instance, gps_device, imu_device, accel_device, gyro_device, compass_device, timestep_val):
        ...
        self.sub_ekf_odom = self.create_subscription(Odometry, '/ekf/odom', self.ekf_odom_callback, 10)
        self.sub_cmd = self.create_subscription(Twist, '/cmd_vel', self.cmd_vel_callback, 10)
        
        self.rpy_publisher = self.create_publisher(Vector3, '/rpy_angles_ekf', 10)
        self.pos_publisher = self.create_publisher(Vector3, '/pos_data_ekf', 10)
    ...
    def publish_imu_compass_data(self):
        ...
        gyro_scale  = np.array([0.01, 0.01, 0.01])  # +1% each
...
while robot.step(timestep) != -1:
    ...
    pitch_rate_pid  = PIDController(kp=0.5, ki=0.000, kd=0.001, dt=dt, name="PitchRate")
    
    roll_angle_pid = PIDController(kp=1.0, ki=0.0, kd=0.08, dt=dt, name="RollAngle")
    roll_rate_pid  = PIDController(kp=0.5, ki=0.000, kd=0.006, dt=dt, name="RollRate")
