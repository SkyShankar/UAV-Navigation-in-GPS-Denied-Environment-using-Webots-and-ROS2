...
class EKFNode(Node):
    ...
    def ekf_step(self):
        ...
        if self.state != 'RUNNING' or self.accel is None or self.gyro is None or self.gps is None or self.mag is None:
        ...
        z = np.array([[x_gps], [y_gps], [z_gps], [roll], [pitch], [yaw]])
        ...
        if self.ekf_step_count % 20 == 0:
            self.get_logger().info(f"EKF Loop Time: {iteration_duration_ms:.3f} ms")
