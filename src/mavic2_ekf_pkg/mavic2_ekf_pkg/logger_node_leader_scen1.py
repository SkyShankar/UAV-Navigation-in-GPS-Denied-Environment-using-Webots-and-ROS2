...
class LoggerNode(Node):
    def __init__(self):
        ...
        self.log_file = os.path.join(log_dir, "ekf_leader_log_scen1.csv")

        with open(self.log_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["time", "ekf_x", "ekf_y", "ekf_z", "x_o", "y_o", "z_o", "roll", "pitch", "yaw", "roll_o", "pitch_o", "yaw_o"])

        self.subscription_ekf = self.create_subscription(Odometry, '/ekf/odom', self.ekf_callback, 10)
        self.rpy_subscriber = self.create_subscription(Vector3,'/rpy_angles_ekf',self.rpy_callback,10)
        self.pos_subscriber = self.create_subscription(Vector3, '/pos_data_ekf', self.pos_callback,10)
