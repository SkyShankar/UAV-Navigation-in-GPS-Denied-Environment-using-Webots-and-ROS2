...
class EKFNode(Node):
    def __init__(self):
        ...
        self.R = np.diag([
            0.7, 0.7, 0.7, 0.0625, 0.0625, 0.002704
        ]) # pseudo-GPS noise and roll pitch yaw noise
        ...
        self.create_subscription(PointStamped, '/gps/fix', self.gps_callback, 10) # GPS from leader
        self.create_subscription(PointStamped, '/target/relative_position', self.relative_position_callback, 10) # Relative position
    ...
    def gps_callback(self, msg):
        ...
        if self.lat_ref is None:
            self.lat_ref = msg.point.x 
            self.lon_ref = msg.point.y
            self.get_logger().info(f"Reference GPS set: x={self.lat_ref}, y={self.lon_ref}") # to know initial coordinate (x,y)

    def relative_position_callback(self, msg):
        self.relative_pos = msg
    ...
    def ekf_step(self):
        ...
        c_phi, s_phi = np.cos(phi_val), np.sin(phi_val)
        c_theta, s_theta = np.cos(theta_val), np.sin(theta_val)
        c_psi, s_psi = np.cos(psi_val), np.sin(psi_val)

        C_bn_numeric = np.array([
            [c_theta * c_psi, -c_theta * s_psi, s_theta],
            [s_phi * s_theta * c_psi + c_phi * s_psi, s_phi * -s_theta * s_psi + c_phi * c_psi, -s_phi * c_theta],
            [-c_phi * s_theta * c_psi + s_phi * s_psi, c_phi * s_theta * s_psi + s_phi * c_psi, c_phi * c_theta]
        ])
        
        # --- pseudo-GPS Correction ---
        x_gps, y_gps, z_gps = self.gps.point.x, self.gps.point.y, self.gps.point.z
        x_rel_pos, y_rel_pos, z_rel_pos = self.relative_pos.point.x, self.relative_pos.point.y, self.relative_pos.point.z
        pos_body = np.array([x_rel_pos, y_rel_pos, z_rel_pos])
        pos_nav_rel = C_bn_numeric.T @ pos_body
        
        x_pseudo_gps = x_gps + pos_nav_rel[0]
        y_pseudo_gps = y_gps + pos_nav_rel[1]
        z_pseudo_gps = z_gps + pos_nav_rel[2]
        ...
        z = np.array([[x_pseudo_gps], [y_pseudo_gps], [z_pseudo_gps], [roll], [pitch], [yaw]])
